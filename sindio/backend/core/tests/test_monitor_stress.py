...
from app.services.monitor.registry import get_config, INFRA_REGISTRY

_calc = StressCalculator()


def _get_stress(infra_type, telemetry, config_capacity):
    config = get_config(infra_type)
    return _calc.compute_stress(config, telemetry, config_capacity)
...
class TestHeuristicStress:
    """Heuristic stress for types without physics engines."""
